...
class cfct_build_admin extends cfct_build_common {
	...
	/**
	 * Add custom row class via ajax
	 *
	 * @param array $args
	 * @return object
	 */
	public function ajax_update_row_class($args) {
		$post_data = $this->get_postmeta($args['post_id']);
		$row_id = $args['id'];
      ...
      $post_data['template']['rows'][$row_id]['bumper-bottom'] = $bumper_bottom;
			...
			$ret = new cfct_message(array(
        'extra' => array('row_id' => $row_id),
				'success' => true,
				'html' => '<div class="cfct-message">'.__('Row Class Saved.', 'carrington-build').'</div>',
				'message' => __('row class saved', 'carrington-build')
			));
		}
		else {
			throw new cfct_template_exception(__('Could not set up Template to update row class','carrington-build').' (post_id: '.$args['post_id'].')');
		}

		do_action('cfct-action-update-row-class', $this, $args['post_id']);
		return $ret;
	}
