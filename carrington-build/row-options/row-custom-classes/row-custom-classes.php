...
class cfct_row_option_custom_classes extends cfct_row_option {
	...
	public function apply_classes($class, $data) {
		...
		return apply_filters('tb_wc_apply_custom_row_options', $data, $build_data['row-options'][$data['guid']][$this->id_base]);
	}
    ...
    $value = apply_filters('tb_wc_list_row_classes', $data, $options_data);
		$html = '<div class="cfct-rendering cfct-row-option-custom-classes">
					<a class="cfct-option cfct-clearfix trigger">
						<div class="class-list'.(!strstr($value,'>none specified<')?' full-width':'').'">Row Classes<br /><span class="row-custom-classes">'.$value.'</span></div>
					</a>
					<div class="form" style="display:none;">
						<div class="class-list">Row Classes</div>
						'.$this->form(isset($options_data['custom-classes'])?$options_data:array('custom-classes' => (isset($data['classes'])?array($data['classes']):array('bg-white')))).'
						<div class="option-note add-class-note">Space seperated</div>
            '.apply_filters('tb_wc_custom_row_options', $data, $options_data).'
    ...
    $post_data['row-options'][$args['row_id']][$this->id_base] = apply_filters('tb_wc_update_custom_row_options', $post_data['row-options'][$args['row_id']][$this->id_base], $args);
