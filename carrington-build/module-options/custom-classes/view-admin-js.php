(function($, cfct_builder) {
    ...
    // Have to be under class_input
    <?php echo apply_filters('tb_custom_module_response_js', false); ?>
		if (ret.data) {
	...
	$(document).on('click', '.cfct-module-option-custom-css .save', function(e) {
		...
		var module_data = {
			'module_id':module_id,
			'data': $this.closest('.form').find('.cfct_custom_class_input').val()<?php
        echo apply_filters('tb_custom_module_options_js', false); ?>
		};
